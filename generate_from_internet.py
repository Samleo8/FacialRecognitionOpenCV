'''
PYTHON CODES (AS ARE MOST IN THIS FOLDER) FROM pyimagesearch.com:
...
#API keys
keys = ["3b29fc93cd9a4ee0bf4deaf6f0be00d4","cfeb0d8645ab441fa5aecb82dcd602a3"]

def generateImages(query):
	output_dir = "dataset/"+query.lower().strip().replace(" ","_")
	...
	# store the search query in a convenience variable then set the
	...
	print("[INFO] {} total results for '{}'".format(estNumResults,query))
	...
	for offset in range(0, estNumResults, GROUP_SIZE):
		...
		for v in results["value"]:
			...
			try:
				...
				p = os.path.sep.join([output_dir,"{}{}".format(str(total).zfill(8), ext)]).split("?")[0]
